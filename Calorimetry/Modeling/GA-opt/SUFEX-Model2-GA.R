...
low <- c(1.0e-18, 1.0e-18, 1.0e-18, -1000.0, -1000.0)
upp <- c(0.2, 0.2, 0.2, 1000.0, 1000.0)
#low <- sapply(par0, function(x) x-0.05*abs(x) )
#upp <- sapply(par0, function(x) x+0.05*abs(x) )

f <- function(x) -log10(myf(x))
GA <- ga(type = "real-valued", fitness = f, lower = low, upper = upp, popSize = 50, maxiter = 20000, run = 100, monitor = gaMonitor, parallel = TRUE)
summary(GA)
plot(GA)
print(GA@solution);
