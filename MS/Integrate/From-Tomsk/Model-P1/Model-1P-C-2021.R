...
p_C <- as.numeric(args[3])

td = read.table(paste0(fname, ".csv"), sep=";", head=TRUE)

pdf(paste0(fname, "-C-1P.pdf"), family="NimbusSan", encoding="KOI8-R.enc")
...
# A0 <- 0.05
 ...
 cap <- sprintf("First order reaction [k=%.2e  C0=%.6f] (Q: %.2e pQ:%.2f)", k, A0, Q, -log10(Q))
